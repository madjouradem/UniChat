...
import 'GroupVeiwFilesController.dart';

class GroupVeiwFiles extends GetView<GroupVeiwFilesCtr> {
  const GroupVeiwFiles({super.key});

  @override
  Widget build(BuildContext context) {
    Get.put(GroupVeiwFilesCtr());
    return SafeArea(
            ...
            bottom: TabBar(
              indicator: BoxDecoration(
                  ...
                  gradient: const LinearGradient(
                    colors: [AppColor.primaryC1, AppColor.primaryC2],
...
class FilesTab extends GetView<GroupVeiwFilesCtr> {
  const FilesTab({
    super.key,
  });
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: GetBuilder<GroupVeiwFilesCtr>(builder: (_) {
          ...
          itemBuilder: (context, index) {
            return GestureDetector(
              onTap: () {
                ...
                fileCtr.openFile(
                    "${controller.groupChatroomCtr.channelCtr.wsFile}/${controller.groupChatroomCtr.channelInfo.chanFile}/${decodeString(controller.filesList[index].mesContent)}");
              },
              child: Container(
                  height: 55,
...
class MediaTab extends GetView<GroupVeiwFilesCtr> {
  const MediaTab({
    super.key,
  });
              ...
              child: controller.mediaList[index].mesType == 'image'
                  ? GestureDetector(
                      onTap: () {
                        Get.toNamed(AppRoute.imageViwer, arguments: {
                          'image': Image.network(
                            "${AppLink.upload}${controller.groupChatroomCtr.channelCtr.wsFile}/${controller.groupChatroomCtr.channelInfo.chanFile}/${decodeString(controller.mediaList[index].mesContent)}",
                          ),
                        });
                      },
                  ...
                  : GetBuilder<VideoFirstFrameCtr>(
                      init: VideoFirstFrameCtr(
                          "${controller.groupChatroomCtr.channelCtr.wsFile}/${controller.groupChatroomCtr.channelInfo.chanFile}/${decodeString(controller.mediaList[index].mesContent)}"),
                      builder: (ctr) {
                        return SizedBox(
                            // height: 200,
                            // width: 200,
...
class LinksTab extends GetView<GroupVeiwFilesCtr> {
  const LinksTab({
    super.key,
  });
